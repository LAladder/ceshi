...
class ShowList(object):
    ...
    def get_header(self):
        # 构建表头
        header_list = []
        print(self.config.new_list_play())
        for field in self.config.new_list_play():
            if callable(field):
                # header_list.append(field.__name__.upper())
...
class ModelStark(object):
    ...
    def checkbox(self, obj=None, header=False):
        if header:
            return mark_safe('<input id="choice" type="checkbox"> 全选')
    ...
    def add_view(self, request):
        ...
        for bfield in form:
            from django.forms.models import ModelChoiceField
            print(type(bfield.field))  # 字段类型
            print(bfield.name)  # 字段名
            print(bfield.field)  # 字段对象
            # 判断是不是多对多类型 或 一对多
            if isinstance(bfield.field, ModelChoiceField):
                bfield.is_pop = True
                print("=================>", bfield.field.queryset.model)  # 一对多 或 多对多 关联模型表
                related_model_name = bfield.field.queryset.model._meta.model_name
                related_app_label = bfield.field.queryset.model._meta.app_label
