...
class CusotmerConfig(ModelStark):
    ...
    def public_customer(self, request):
        ...
        # __lt 小于  三天未跟进  # 15天未成单   exclude 按条件排除
        user_id = 5
    ...
    def further(self, request, customer_id):
        ...
        delta_day15 = datetime.timedelta(days=15)
        # 未该客户 更改课程顾问 和对应的时间
        ret = Customer.objects.filter(pk=customer_id).filter(Q(last_consult_date__lt=now - delta_day3) | Q(recv_date__lt=now - delta_day15), status=2).update(consultant=user_id,recv_date=now, last_consult_date=now)
        if not ret:
    ...
    def score(self, request, course_record_id):
        if request.method == "POST":
            ...
            for key, value in request.POST.items():
                ...
                # 从右往左  分割
                field, pk = key.rsplit("_", 1)  # homework_note_4     [homework_note, 4]
